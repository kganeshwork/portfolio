...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
volatile uint8_t rb_buf[RINGBUF_SIZE + 1];
ringbuf rb = {.len = RINGBUF_SIZE, .buf = rb_buf, .pos = 0, .ext = 0};
/* USER CODE END 0 */
...
void UART_CharReception_Callback(void)
{
   ...
   * register */
  ringbuf_write(rb, LL_USART_ReceiveData8(USARTx));
...
void UART_Error_Callback(void)
{
  ...
  /* Error handling example :
        ...
        */
  sr_reg = LL_USART_ReadReg(USARTx, ISR);
  if (sr_reg & LL_USART_ISR_NE)
  {
  ...
  {
    /* Turn LED3 on: Transfer error in reception/transmission
     * process */
